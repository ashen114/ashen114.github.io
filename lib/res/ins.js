  ...
  [
    ...
    function(module, exports, __webpack_require__) {
      ...
      var createVideoIncon = function createVideoIncon() {
        /** 获取文档中 class="thumb" a[data-type="video"]的所有元素: */
      ...
      var render = function render(res) {
        ...
        for (var j = 0, len2 = res.list.length; j < len2; j++) {
          ...
          for (var i = 0, len = data.link.length; i < len; i++) {
            ...
            var minSrc =
              "https://raw.githubusercontent.com/ashen114/hexo_res/master/min_photos/" +
              data.link[i];

            // var src = 'https://raw.githubusercontent.com/yourgithub/Blog_Album/master/photos/' + data.link[i];
            var src =
              "https://raw.githubusercontent.com/ashen114/hexo_res/master/photos/" +
              data.link[i];
            ...
            liTmpl +=
              ...
              '" data-src="' +
              minSrc +
              '" src="../lib/res/assets/empty.png" itemprop="thumbnail" onload="lzld(this)">\
                  </a>\
      ...
      function loadData(success) {
        if (!searchData) {
          ...
          xhr.open("GET", "../lib/res/data.json?t=" + +new Date(), true);
      ...
      var initPhotoSwipeFromDOM = function initPhotoSwipeFromDOM(
        ...
        var parseThumbnailElements = function parseThumbnailElements(el) {
          ...
          for (var i = 0; i < numNodes; i++) {
            ...
            // 判断图片比例，若比例相近，则10倍数放大，若相差10倍则100倍放大

            console.log(
              "figureEl.clientWidth/figureEl.clientHeight",
              figureEl.clientWidth / figureEl.clientHeight
            );
            let wDivd = figureEl.clientWidth / figureEl.clientHeight,
              wdPow = 10;
            if (wDivd >= 1) {
              if (wDivd >= 2) {
                // 宽大于高
                wdPow = Math.pow(10, Math.floor(wDivd));
              }
            } else {
              // 高大于宽
              wdPow = Math.pow(10, wDivd.toString().split(".")[1].length);
            }
            item = {
              src: linkEl.getAttribute("href"),
              w: figureEl.clientWidth * wdPow || parseInt(size[0], 10),
              h: figureEl.clientHeight * wdPow || parseInt(size[1], 10)
            };
